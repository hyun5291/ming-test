...
function Kimchart() {
    ...
    async function getTodos() {
        try {
            const {data, error} = await supabase.from("kimp").select("*").order("id", {ascending: true});

            if (error) {
                return [];
            }

            console.log("getTodosdata>", data);
            setTb_data(data);
        } catch (err) {
            console.error(err);
        }
    ...
    const handleInsert = async (val: any) => {
        ...
        const {error} = await supabase.from("kimp").insert([{amount: val}]);
        if (error) {
            console.error(error);
            alert("등록 실패");
        } else {
    ...
    const handleDelete = async (val: any, e: any) => {
        ...
        const {error} = await supabase.from("kimp").delete().eq("id", val);
        if (error) {
            console.error(error);
            alert("삭제 실패");
        } else {
